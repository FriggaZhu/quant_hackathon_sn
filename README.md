...
ROOSTOO_PAIRS=BTC/USD,ETH/USD,SOL/USD,TRX/USD
ROOSTOO_POLL_INTERVAL=900
ROOSTOO_HISTORY_SIZE=100
ROOSTOO_ENABLE_TRADING=false
ROOSTOO_STRATEGY=pair_tuned_mean_reversion
...
ROOSTOO_DAILY_TRADE_TRIGGER_HOUR_UTC=22
ROOSTOO_DAILY_TRADE_TRIGGER_MINUTE_UTC=0
ROOSTOO_DAILY_TRADE_FALLBACK_PAIR=SOL/USD
...
ROOSTOO_MR_WEAK_BUY_FRACTION_PCT=0.00
...
ROOSTOO_MR_TREND_EXTENSION_RSI_THRESHOLD=54
ROOSTOO_MR_TREND_EXTENSION_TRAILING_STOP_PCT=0.010
ROOSTOO_MR_TREND_EXTENSION_SELL_FRACTION_PCT=
ROOSTOO_MR_REQUIRE_PRICE_ABOVE_TREND_EMA_FOR_ENTRY=false
ROOSTOO_MR_ENTRY_BELOW_TREND_EMA_BUFFER_PCT=0.0
```

Optional per-asset mean-reversion overrides:

```text
ROOSTOO_STRATEGY=pair_tuned_mean_reversion
ROOSTOO_PAIRS=BTC/USD,ETH/USD,SOL/USD,TRX/USD
ROOSTOO_MR_MAX_VOLATILITY=0.012
ROOSTOO_MR_TREND_EXTENSION_TRAILING_STOP_PCT=0.009
ROOSTOO_PAIRCFG_SOL_USD_MR_TREND_EXTENSION_TRAILING_STOP_PCT=0.010
```

This uses the normal `ROOSTOO_MR_*` settings as defaults and only overrides the pairs you specify.

Optional trend-extension exit experiment:

```text
ROOSTOO_MR_TREND_EXTENSION_SELL_FRACTION_PCT=0.50
```

If set, only trend-extension exits use the smaller sell fraction. Normal mean-reversion exits still use `ROOSTOO_PORTFOLIO_SELL_FRACTION_PCT`.

For the current best live candidate plus the daily-trade requirement overlay, use the pair-tuned `BTC/USD, ETH/USD, SOL/USD, TRX/USD` setup and add:
